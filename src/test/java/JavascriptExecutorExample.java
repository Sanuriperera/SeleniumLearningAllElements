...
//        jsExecutor.executeScript("alert('my name is sanuri');");

        //2) Set a input value in a text box using javascriptExecutor
        //2.1) - way 1 > Set the value using the value property (common approach and recommended approach )
        WebElement inputNameTextBox = driver.findElement(By.xpath("//input[@id='name']"));
//        jsExecutor.executeScript("arguments[0].value='My name is Sanuri';", inputNameTextBox);

        //2.2) - way 2 > Set the value using setAttribute (alternative approach)
        jsExecutor.executeScript("arguments[0].setAttribute('value', 'My name is Sanuri');", inputNameTextBox);
        Thread.sleep(3000);

        //3) Highlight element (only using javasrcipt executor)
